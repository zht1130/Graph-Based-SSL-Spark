...
case class adsorption (g:Graph[knnVertex,Double], c:Int) {
...
//        .reduce(_ || _)
  ...
  def multiply(p:Double, m:HashMap[Int, Double]) = {

    m.map{ case (k,v) => k -> p * v }
  }

  def dummyLabels(c:Int) = {

    var m = HashMap[Int, Double]()
