...
with KNNModelParams with HasWeightCol with Serializable {
    ...
    val merged = neighborRDD
      .map {
        case (id, idsDists) =>
          ...
          val a = ids.map(_.getLong(0)).zip(dists).mkString(" ")

          val vector = new Array[Double](5).mkString(" ")

          val rawPrediction = Vectors.dense(1)
          lazy val probability = Vectors.dense(1)
          lazy val prediction = a
  ...
  override def transformSchema(schema: StructType): StructType = {
    ...
    if ($(predictionCol).nonEmpty) {
      transformed = SchemaUtils.appendColumn(transformed, $(predictionCol), StringType)
    }
    transformed
  }
