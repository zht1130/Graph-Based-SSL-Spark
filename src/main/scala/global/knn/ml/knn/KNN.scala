...
class KNNModel private[ml](
  ...
  def edges(dataset: Dataset[_]) = {

    val neighborRDD : RDD[(Long, Array[(Row, Double)])] = transform(dataset, topTree, subTrees)

    neighborRDD.map {
      case (id, idsDists) =>
        val (ids, dists) = idsDists.unzip
        ids.map(_.getLong(0))
          .zip(dists.map(d => 1 / (1+d)))
          .filter(id != _._1)
          .map(x => Edge(id, x._1, x._2))
    }.flatMap(y => y)
  }
...
/**
  * k-Nearest Neighbors (kNN) local.algorithm
  *
  * kNN finds k closest observations in training dataset. It can be used for both classification and regression.
  * Furthermore it can also be used for other purposes such as input to clustering local.algorithm.
  *
  ...
  * Each local.algorithm has its shortcomings that prevent them to be effective on large-scale and/or high-dimensional dataset.
  *
  * This is an implementation of kNN based upon distributed Hybrid Spill-Trees where training points are organized into
  * distributed binary trees. The local.algorithm is designed to support accurate approximate kNN search but by tuning parameters
  ...
  *
  * A high level overview of the local.algorithm is as follows:
  *
  ...
  *
  *
  * This local.algorithm is described in [[http://dx.doi.org/10.1109/WACV.2007.18]] where it was shown to scale well in terms of
  * number of observations and dimensions, bounded by the available memory across clusters (billions in paper's example).
  * This implementation adapts the MapReduce local.algorithm to work with Spark.
  *
  */
...
////      .map(row => new RowWithVector(row.getAs[Vector](0), row.getStruct(1)))
    ...
    val data = dataset.select("id").rdd.zip(dataset.select($(featuresCol)).rdd).map(x => new RowWithVector(x._2.getAs[Vector](0), x._1))
...
object KNN {
  ...
  /**
    * VectorWithNorm can use more efficient local.algorithm to calculate distance
    */
  case class VectorWithNorm(vector: Vector, norm: Double) {
    def this(vector: Vector) = this(vector, Vectors.norm(vector, 1))
